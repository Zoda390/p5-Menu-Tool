...
function preload(){
    loadedUiImgs.push(loadImage("emptyImage.png"));
    loadedUiImgs[loadedUiImgs.length-1].src = "emptyImage.png";
}

function setup(){
    socket = io.connect("http://localhost:3000");
    socket.on("fileReply", (data)=>{
        console.log("got file reply");
        loadedUiImgs.push(loadImage('assets/uiImg'+data.id+'.png'));
        loadedUiImgs[loadedUiImgs.length-1].src = 'assets/uiImg'+data.id+'.png';
        imageList.updatehtml();
    });
    ps = new Page_Sorter();
    mainDiv = createDiv();
    ...
    leftMenu = createDiv(`
        ...
        <div class="dropdown-content">
            <button onclick="objects.push(new Box(100, 100, 100, 100, 200,100,100));">Add Box</button>
            <button onclick="objects.push(new Txt('bla', 100, 100, 0,0,0));">Add Text</button>
            <button onclick="objects.push(new Img(0, 100, 100, 100, 100));">Add Img</button>
        </div>
    `);
    ...
    boxSettingsDiv = createDiv(`
        ...
        <input type="color" id="fillColor" name="fillColor" oninput="updateColor(this.value, 'box')" style="background-color: #4c4c4c; color: white; font-size: x-large;">
        <label for="width" style="margin-left: 10px; font-size: x-large;">Box Width:</label>
        <input type="number" id="width" name="width" oninput="updateWidth(this.value, 'box')" style="background-color: #4c4c4c; color: white; font-size: x-large;">
        <label for="height" style="margin-left: 10px; font-size: x-large;">Box Height:</label>
        <input type="number" id="height" name="height" oninput="updateHeight(this.value, 'box')" style="background-color: #4c4c4c; color: white; font-size: x-large;">
        </div>
    `);
    ...
    txtSettingsDiv = createDiv(`
        ...
        <label for="fillColor" style="margin-left: 10px; font-size: x-large;">Fill Color:</label>
        <input type="color" id="fillColor" name="fillColor" oninput="updateColor(this.value, 'txt')" style="background-color: #4c4c4c; color: white; font-size: x-large;">
    ...
    imgSettingsDiv = createDiv(`
        <h1 style="margin: 5px;">Image Settings:</h1>
        <div>
        <label style="margin-left: 10px; font-size: x-large;">Image: 0</label>
        <label for="width" style="margin-left: 10px; font-size: x-large;">Image Width:</label>
        <input type="number" id="width" name="width" oninput="updateWidth(this.value, 'img')" style="background-color: #4c4c4c; color: white; font-size: x-large;">
        <label for="height" style="margin-left: 10px; font-size: x-large;">Image Height:</label>
        <input type="number" id="height" name="height" oninput="updateHeight(this.value, 'img')" style="background-color: #4c4c4c; color: white; font-size: x-large;">
        </div>
    `);
    imgSettingsDiv.style("color: white");
    imgSettingsDiv.parent(bottomDiv);
    imgSettingsDiv.position(0,0);
    imgSettingsDiv.hide();

    imageList = new p5_Image_List();
    imageList.html.parent(imgSettingsDiv);
    imageList.pics = loadedUiImgs;
    imageList.updatehtml();

    imgSettingsDiv.elt.innerHTML += `
        <input type="file" oninput="updateImgArr(this.files[0])" style="background-color: #4c4c4c; color: white; font-size: x-large;">
    `;
...
function updateImgArr(val){
    //console.log(val);
    const reader = new FileReader();
    reader.addEventListener('load', (e)=>{
        console.log("loaded");
        socket.emit("getFile", {src: e.target.result.replace(/^data:image\/png;base64,/, ""), id: loadedUiImgs.length});
    });
